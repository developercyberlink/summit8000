...
class PaymentService extends ActionRequestService
{
    /**
    ...
    public function ExecuteFormJose($mid,$api_key,$curr,$amt,$threeD,$success_url,$failed_url,$cancel_url,$backend_url,$refId,$productName): string
    {
        $now = Carbon::now();
        $orderNo = $refId;
        $request = [
        ...
        $decryptingKey = $this->GetPrivateKey(SecurityData::$MerchantDecryptionPrivateKey);
        $signatureVerificationKey = $this->GetPublicKey(SecurityData::$PacoSigningPublicKey);
        // dd($this->DecryptToken($token, $decryptingKey, $signatureVerificationKey));
        return $this->DecryptToken($token, $decryptingKey, $signatureVerificationKey);
    }
}