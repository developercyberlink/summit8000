...
@section('breadcrumb')
<a href="{{ url('admin/trip/'.Request::segment(3).'/create') }}" class="btn btn-primary btn-sm">Create</a>
@endsection
            ...
            <tr class="id{{$row->id}}">
              ...
              <td class="post_title title_hi_sh"> 
                @if(check_child_post($row->id))
                    ...
                    <strong> {{ ucfirst($row->title) }} </strong> 
                @endif                
                <div class="row_actions"><span class="id">ID: {{$row->id}} | </span><span class="edit"><a href="{{url( 'admin/trip/'.Request::segment(3).'/'.$row->id.'/edit')}}" aria-label="">Edit</a> 
                 </span>                 
...
(function ( $ ) { 
  $('.submitdelete1').on('click',function(e){
    ...
    $.ajax({
      type:'delete',
      url:"{{url('admin/trip').'/'.Request::segment(3).'/' }}" + id,
      data:{_token:csrf},    
