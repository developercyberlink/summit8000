...
@section('title', '')
@section('breadcrumb')
<a href="{{ route('admin.testimonials.create',$trip_id) }}" class="btn btn-primary btn-sm">Create</a>
@endsection
@section('content')
				...
				<table class="table admin-form theme-warning fs13">
						...
						<tbody>
							...
							<tr class="id{{$row->id}}">
								...
								<td class="text-left">  
									<a href="{{ route('admin.testimonials.edit',['admin'=>$trip_id,'testimonial'=>$row->id]) }}">Edit</a> 
...
jQuery(document).ready(function() {
  $('.btn-delete').on('click',function(e){
    ...
    var trip_id = '{{$trip_id}}';
    var url = '{{ route("admin.testimonials.destroy",["admin"=>':admin',"testimonial"=>':testimonial']) }}';
