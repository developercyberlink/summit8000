...
@section('title','Training Enrollment')
@section('breadcrumb')
@endsection
@section('content')
                        ...
                        @if(count($enrollment) > 0)
                            @foreach($enrollment as $key=>$row)
                                <tr class="bg-light">
                                    ...
                                    <td class="">{{ ucfirst($row->training_title) }}<br>Price: {{$row->price}}<br>Start Date: {{$row->start_date}}<br>Duration: {{$row->duration}} Day(s)</td>                 
                                    <td class="">{{$row->email}}<br>Phone: {{$row->contact}}</td> 
                                    <td class=""><textarea>{!!$row->message!!}</textarea></td> 
                                    <td class="text-left">
                                        <form action="{{ route('training-enrollment.destroy', $row->id) }}" method="POST" onsubmit="return confirmDelete()">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
